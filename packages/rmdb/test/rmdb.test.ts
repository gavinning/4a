...
describe('RMDB Class test', () => {
  ...
  it('test base', async () => {
    const arr = [1, 2, 3]
    const store = creator('test:f1').from(() => arr)
  ...
  it('test filters', async () => {
    ...
    function getPostsHandler(user: User) {
      return creator('app:foo', user.id).keep(5, 10).from(getPosts).get(user)
    }
